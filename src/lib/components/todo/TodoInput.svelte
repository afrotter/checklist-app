...
<div>
    <div>
        <label for={`${todo.id}-checkbox`}>Complete todo</label>
        <input id={`${todo.id}-checkbox`} type="checkbox" checked={todo.complete} on:change={() => toggleComplete()}/>
    
        <label for={`${todo.id}-text`}>Edit todo</label>
        <input
			id={`${todo.id}-text`}
			type="text"
